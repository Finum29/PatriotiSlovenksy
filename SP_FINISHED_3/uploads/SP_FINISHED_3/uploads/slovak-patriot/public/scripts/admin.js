...
const EVENT_IMAGES = {
  ...
  'lol': '/images/LeagueofLegends.jpg',
  'f1': '/images/F1.jpg'
};
...
function showModal(title, message, onConfirm = null, onCancel = null) {
  const modalHtml = `
    <div id="customModal" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); display: flex; align-items: center; justify-content: center; z-index: 10000;">
      <div style="background: var(--black-light); padding: 30px; border-radius: 12px; max-width: 500px; width: 90%; border: 2px solid var(--accent-red);">
        ...
        <div style="display: flex; gap: 10px; justify-content: flex-end;">
          ${onCancel ? '<button id="modalCancel" class="btn btn-delete" style="padding: 10px 20px;">Cancel</button>' : ''}
...
function showInputModal(title, message, defaultValue = '', onConfirm = null) {
  const modalHtml = `
    <div id="customModal" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); display: flex; align-items: center; justify-content: center; z-index: 10000;">
      <div style="background: var(--black-light); padding: 30px; border-radius: 12px; max-width: 500px; width: 90%; border: 2px solid var(--accent-red);">
        <h3 style="color: var(--accent-red); margin-bottom: 15px;">${title}</h3>
        <p style="color: var(--white); margin-bottom: 15px;">${message}</p>
        <input type="text" id="modalInput" value="${defaultValue}" style="width: 100%; padding: 12px; border: 2px solid var(--black-lighter); border-radius: 6px; background: var(--black); color: var(--white); font-size: 1rem; margin-bottom: 20px;">
        <div style="display: flex; gap: 10px; justify-content: flex-end;">
          <button id="modalCancel" class="btn btn-delete" style="padding: 10px 20px;">Cancel</button>
          <button id="modalConfirm" class="btn btn-activate" style="padding: 10px 20px;">OK</button>
        </div>
      </div>
    </div>
...
async function addCreditsToUser(userId, username) {
  showInputModal('Add Credits', `How many credits do you want to ADD to ${username}?`, '', (amount) => {
    if (!amount || isNaN(amount) || parseInt(amount) <= 0) {
      showModal('Invalid Input', 'Please enter a valid positive number');
      return;
    }
    
    showInputModal('Reason', 'Enter reason for adding credits (optional):', 'Admin adjustment', async (reason) => {
      try {
        ...
        if (data.ok) {
          showModal('Success', data.message, () => loadPlayers());
        } else {
          showModal('Error', data.message || 'Failed to add credits');
        }
      } catch (error) {
        console.error('Error adding credits:', error);
        showModal('Error', 'Failed to add credits');
      }
    });
  });
}
...
async function removeCreditsFromUser(userId, username) {
  showInputModal('Remove Credits', `How many credits do you want to REMOVE from ${username}?`, '', (amount) => {
    if (!amount || isNaN(amount) || parseInt(amount) <= 0) {
      showModal('Invalid Input', 'Please enter a valid positive number');
      return;
    }
    
    showInputModal('Reason', 'Enter reason for removing credits (optional):', 'Admin adjustment', (reason) => {
      showModal('Confirm', `Are you sure you want to remove ${amount} credits from ${username}?`, async () => {
        try {
          ...
          if (data.ok) {
            ...
            showModal('Error', data.message || 'Failed to remove credits');
          }
        } catch (error) {
          console.error('Error removing credits:', error);
          showModal('Error', 'Failed to remove credits');
        }
      });
    });
  });
...
async function banUser(userId, username) {
  showModal('Confirm Ban', `Are you sure you want to BAN ${username}? They will not be able to access the site.`, async () => {
    try {
      ...
      if (data.ok) {
        showModal('Success', `${username} has been banned`, () => loadPlayers());
      }
    } catch (error) {
...
async function suspendUser(userId, username) {
  showModal('Confirm Suspend', `Are you sure you want to SUSPEND ${username}? They won't be able to register for events.`, async () => {
    try {
      ...
      if (data.ok) {
        showModal('Success', `${username} has been suspended`, () => loadPlayers());
      }
    } catch (error) {
...
async function activateUser(userId, username) {
  showModal('Confirm Activation', `Activate ${username}'s account?`, async () => {
    try {
      ...
      if (data.ok) {
        showModal('Success', `${username} has been activated`, () => loadPlayers());
      }
    } catch (error) {
...
async function deleteUser(userId, username) {
  showModal('Confirm Delete', `Are you sure you want to DELETE ${username}'s account? This cannot be undone.`, async () => {
    try {
      ...
      if (data.ok) {
        showModal('Success', `${username}'s account has been deleted`, () => loadPlayers());
      }
    } catch (error) {
...
async function showDisqualifyModal(userId, username) {
  ...
  if (!eventsData.ok || eventsData.events.length === 0) {
    showModal('No Events', 'No events available for disqualification');
    return;
  }
  ...
  showInputModal('Disqualify User', `Select event to disqualify ${username} from:\n\n${eventOptions}\n\nEnter event name:`, '', async (eventName) => {
    if (!eventName) return;

    const event = eventsData.events.find(e => e.name === eventName);
    if (!event) {
      showModal('Error', 'Invalid event name');
      return;
    }
...
async function disqualifyUser(userId, username, eventId) {
  try {
    ...
    if (data.ok) {
      showModal('Success', `${username} has been disqualified from the event. Entry fee is NOT refunded.`, () => {
        loadPlayers();
        loadEvents();
      });
    }
...
async function deleteTeam(teamId, teamName) {
  showModal('Confirm Delete', `Are you sure you want to DELETE team "${teamName}"? All members will be removed from the team. This cannot be undone.`, async () => {
    try {
      ...
      if (data.ok) {
        showModal('Success', `Team "${teamName}" has been deleted`, () => {
          loadTeams();
          loadPlayers();
        });
      }
      ...
      showModal('Error', 'Failed to delete team');
    }
  });
}
    ...
    <div class="admin-card" draggable="true" ondragstart="drag(event)" id="event-${event.id}" data-event-id="${event.id}">
      ...
      <div class="card-body">
        ...
        <div style="margin-top: 10px;">
          <img src="${event.iconUrl || '/images/Event.jpg'}" alt="${event.name}" style="max-width: 200px; border-radius: 8px;" onerror="this.src='/images/Event.jpg'">
        </div>
      </div>
      <div class="card-actions">
...
async function showAwardPrizeModal(eventId) {
  ...
  showInputModal('Award Prize', `Select participant to award prize:\n\n${participants}\n\nEnter name:`, '', (winnerName) => {
    if (!winnerName) return;

    showInputModal('Prize Amount', 'Enter prize amount (credits):', '', async (amount) => {
      ...
      if (!registration) {
        showModal('Error', 'Invalid participant name');
        return;
      }
      ...
      try {
        ...
        if (data.ok) {
          showModal('Success', `Awarded ${amount} credits to ${winnerName}`, () => loadEvents());
        }
      } catch (error) {
        console.error('Error awarding prize:', error);
        showModal('Error', 'Failed to award prize');
      }
    });
  });
}
...
async function setEventImage(eventId, currentUrl) {
  showInputModal('Set Event Image', 'Enter event image URL:', currentUrl, async (iconUrl) => {
    ...
    try {
      ...
      if (data.ok) {
        showModal('Success', 'Event image updated successfully!', () => loadEvents());
      }
    } catch (error) {
      console.error('Error setting event image:', error);
      showModal('Error', 'Failed to set event image');
    }
  });
}

async function setStreamUrl(eventId, currentUrl) {
  showInputModal('Set Stream URL', 'Enter live stream URL (YouTube, Twitch, etc.):', currentUrl, async (streamUrl) => {
    ...
    try {
      ...
      if (data.ok) {
        showModal('Success', 'Stream URL set successfully!', () => loadEvents());
      }
    } catch (error) {
      console.error('Error setting stream URL:', error);
      showModal('Error', 'Failed to set stream URL');
    }
  });
}

async function setLobbyUrl(eventId, currentUrl) {
  showInputModal('Set Lobby URL', 'Enter lobby URL (e.g., Discord invite, game server link):', currentUrl, async (lobbyUrl) => {
    ...
    try {
      ...
      if (data.ok) {
        showModal('Success', 'Lobby URL set successfully!', () => loadEvents());
      }
    } catch (error) {
      console.error('Error setting lobby URL:', error);
      showModal('Error', 'Failed to set lobby URL');
    }
  });
}
...
async function announceWinner(eventId) {
  ...
  showInputModal('Announce Winner', `Select winner:\n\n${participants}\n\nEnter winner name:`, '', async (winnerName) => {
    if (!winnerName) return;
    ...
    if (!registration) {
      showModal('Error', 'Invalid winner name');
      return;
    }
    ...
    try {
      ...
      if (data.ok) {
        showModal('Success', `Winner announced: ${winnerName}`, () => {
          loadEvents();
          loadEventManagement();
        });
      ...
      showModal('Error', 'Failed to announce winner');
    }
  });
}
...
async function updateEventStatus(eventId, status) {
  try {
    ...
    if (data.ok) {
      showModal('Success', `Event status updated to ${status}`, () => {
        loadEvents();
        loadEventManagement();
      });
...
async function deleteEvent(eventId, eventName) {
  showModal('Confirm Delete', `Are you sure you want to delete "${eventName}"? This cannot be undone.`, async () => {
    try {
...
async function closeTicket(ticketId) {
  showModal('Confirm', 'Close this ticket?', async () => {
    try {
      ...
      if (data.ok) {
        showModal('Success', 'Ticket closed', () => loadTickets());
      }
    } catch (error) {
...
async function reopenTicket(ticketId) {
  showModal('Confirm', 'Reopen this ticket?', async () => {
    try {
      ...
      if (data.ok) {
        showModal('Success', 'Ticket reopened', () => loadTickets());
      }
    } catch (error) {
...
function displayEventManagement(events) {
  ...
  eventManagement.innerHTML = events.map(event => {
    ...
    if (event.bracket) {
        bracketHtml = `
            <div class="bracket-admin-view" style="margin-top: 20px; overflow-x: auto; background: var(--black); padding: 20px; border-radius: 10px;">
                <h4 style="color: var(--accent-red); margin-bottom: 15px; font-size: 1.3rem;">🏆 Winner Bracket</h4>
                <div style="display: flex; gap: 30px; padding: 10px;">
                    ${event.bracket.map((round, rIndex) => `
                        <div class="round-column" style="min-width: 250px;">
                            <h5 style="text-align: center; margin-bottom: 15px; color: var(--white); font-size: 1.1rem; padding: 10px; background: var(--black-lighter); border-radius: 6px;">Round ${rIndex + 1}</h5>
                            ${round.map(match => `
                                <div class="match-card" style="background: var(--black-light); padding: 15px; margin-bottom: 15px; border-radius: 8px; border: 2px solid ${match.winner ? '#4CAF50' : 'var(--gray)'}; box-shadow: 0 4px 6px rgba(0,0,0,0.3);">
                                    <div style="font-size: 0.85rem; color: #888; margin-bottom: 10px; font-weight: bold;">${match.matchId}</div>
                                    
                                    <div class="participant p1" onclick="sendLobbyLink('${event.id}', '${match.participant1?.id}', '${match.participant1?.name}')" style="cursor: pointer; padding: 10px; margin-bottom: 8px; border-radius: 4px; background: ${match.winner?.id === match.participant1?.id ? 'rgba(76, 175, 80, 0.2)' : 'var(--black)'}; border-left: 4px solid ${match.winner?.id === match.participant1?.id ? '#4CAF50' : '#666'}; transition: all 0.3s;">
                                        <span style="color: ${match.winner?.id === match.participant1?.id ? '#4CAF50' : 'var(--white)'}; font-weight: ${match.winner?.id === match.participant1?.id ? 'bold' : 'normal'};">
                                            ${match.participant1 ? `🎮 ${match.participant1.name}` : '⏳ TBD'}
                                        </span>
                                    </div>
                                    
                                    <div class="participant p2" onclick="sendLobbyLink('${event.id}', '${match.participant2?.id}', '${match.participant2?.name}')" style="cursor: pointer; padding: 10px; border-radius: 4px; background: ${match.winner?.id === match.participant2?.id ? 'rgba(76, 175, 80, 0.2)' : 'var(--black)'}; border-left: 4px solid ${match.winner?.id === match.participant2?.id ? '#4CAF50' : '#666'}; transition: all 0.3s;">
                                        <span style="color: ${match.winner?.id === match.participant2?.id ? '#4CAF50' : 'var(--white)'}; font-weight: ${match.winner?.id === match.participant2?.id ? 'bold' : 'normal'};">
                                            ${match.participant2 ? `🎮 ${match.participant2.name}` : '⏳ TBD'}
                                        </span>
                                    </div>

                                    <div style="margin-top: 12px; display: flex; gap: 8px; justify-content: center;">
                                        <button onclick="quickUpdateResult('${event.id}', '${match.matchId}', '${match.participant1?.id}')" style="flex: 1; padding: 8px; font-size: 0.85rem; background: #4CAF50; color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: bold;">✓ P1 Wins</button>
                                        <button onclick="quickUpdateResult('${event.id}', '${match.matchId}', '${match.participant2?.id}')" style="flex: 1; padding: 8px; font-size: 0.85rem; background: #4CAF50; color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: bold;">✓ P2 Wins</button>
                                    </div>
                                </div>
                            `).join('')}
                        </div>
                ...
                ${event.loserBracket ? `
                <h4 style="color: var(--accent-red); margin: 30px 0 15px; font-size: 1.3rem;">🎯 Loser Bracket</h4>
                <div style="display: flex; gap: 30px; padding: 10px;">
                    ${event.loserBracket.map((round, rIndex) => `
                        <div class="round-column" style="min-width: 250px;">
                            <h5 style="text-align: center; margin-bottom: 15px; color: var(--white); font-size: 1.1rem; padding: 10px; background: var(--black-lighter); border-radius: 6px;">LB Round ${rIndex + 1}</h5>
                            ${round.map(match => `
                                <div class="match-card" style="background: var(--black-light); padding: 15px; margin-bottom: 15px; border-radius: 8px; border: 2px solid ${match.winner ? '#FF9800' : 'var(--gray)'}; box-shadow: 0 4px 6px rgba(0,0,0,0.3);">
                                    <div style="font-size: 0.85rem; color: #888; margin-bottom: 10px; font-weight: bold;">${match.matchId}</div>
                                    
                                    <div class="participant p1" style="padding: 10px; margin-bottom: 8px; border-radius: 4px; background: ${match.winner?.id === match.participant1?.id ? 'rgba(255, 152, 0, 0.2)' : 'var(--black)'}; border-left: 4px solid ${match.winner?.id === match.participant1?.id ? '#FF9800' : '#666'};">
                                        <span style="color: ${match.winner?.id === match.participant1?.id ? '#FF9800' : 'var(--white)'}; font-weight: ${match.winner?.id === match.participant1?.id ? 'bold' : 'normal'};">
                                            ${match.participant1 ? `🎮 ${match.participant1.name}` : '⏳ TBD'}
                                        </span>
                                    </div>
                                    
                                    <div class="participant p2" style="padding: 10px; border-radius: 4px; background: ${match.winner?.id === match.participant2?.id ? 'rgba(255, 152, 0, 0.2)' : 'var(--black)'}; border-left: 4px solid ${match.winner?.id === match.participant2?.id ? '#FF9800' : '#666'};">
                                        <span style="color: ${match.winner?.id === match.participant2?.id ? '#FF9800' : 'var(--white)'}; font-weight: ${match.winner?.id === match.participant2?.id ? 'bold' : 'normal'};">
                                            ${match.participant2 ? `🎮 ${match.participant2.name}` : '⏳ TBD'}
                                        </span>
                                    </div>

                                    <div style="margin-top: 12px; display: flex; gap: 8px; justify-content: center;">
                                        <button onclick="quickUpdateResult('${event.id}', '${match.matchId}', '${match.participant1?.id}')" style="flex: 1; padding: 8px; font-size: 0.85rem; background: #FF9800; color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: bold;">✓ P1 Wins</button>
                                        <button onclick="quickUpdateResult('${event.id}', '${match.matchId}', '${match.participant2?.id}')" style="flex: 1; padding: 8px; font-size: 0.85rem; background: #FF9800; color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: bold;">✓ P2 Wins</button>
                                    </div>
                                </div>
                            `).join('')}
                        </div>
                ...
                ` : ''}
                <p style="font-size: 0.9rem; color: var(--gray); margin-top: 20px; padding: 10px; background: var(--black-lighter); border-radius: 6px;">💡 <strong>Tip:</strong> Click on a participant name to send them a lobby link.</p>
            </div>
        `;
    }
...
async function sendLobbyLink(eventId, participantId, participantName) {
    ...
    showInputModal('Send Lobby Link', `Send lobby link to ${participantName}:`, '', async (lobbyLink) => {
        if (!lobbyLink) return;
        ...
        try {
            ...
            showModal('Success', `Lobby link sent to ${participantName}`);
        } catch (error) {
            console.error('Error sending lobby link:', error);
            showModal('Error', 'Failed to send link');
        }
    });
}
...
async function quickUpdateResult(eventId, matchId, winnerId) {
    ...
    showModal('Confirm Winner', 'Confirm winner?', async () => {
        try {
            ...
            showModal('Error', 'Failed to update match result');
          }
    });
}

async function generateBracket(eventId) {
  showModal('Confirm', 'Generate tournament bracket for this event? This will automatically seed all registered participants.', async () => {
    try {
      ...
      if (data.ok) {
        showModal('Success', 'Bracket generated successfully!', () => loadEventManagement());
      }
    } catch (error) {
      console.error('Error generating bracket:', error);
      showModal('Error', 'Failed to generate bracket');
    }
  });
}
...
async function scheduleMatches(eventId) {
  ...
  if (!event || !event.bracket) {
    showModal('Error', 'Event or bracket not found');
    return;
  }

  showInputModal('Schedule Match', 'Enter match ID to schedule (e.g., R0-M0):', '', (matchId) => {
    if (!matchId) return;

    showInputModal('Scheduled Time', 'Enter scheduled time (YYYY-MM-DD HH:MM):', '', async (scheduledTime) => {
      ...
      try {
        ...
        if (data.ok) {
          showModal('Success', 'Match scheduled successfully!', () => loadEventManagement());
        }
      } catch (error) {
        console.error('Error scheduling match:', error);
        showModal('Error', 'Failed to schedule match');
      }
    });
  });
}
...
async function editBracket(eventId) {
    ...
    showInputModal('Edit Bracket', 'Enter match ID to edit (e.g., R0-M0):', '', async (matchId) => {
      if (!matchId) return;
      ...
      if (!match) {
        showModal('Error', 'Match not found');
        return;
      }
      ...
      showInputModal('Select Participant 1', `Select Participant 1 (Enter number, or 0 for empty):\n\n${pList}`, '', (p1Index) => {
        showInputModal('Select Participant 2', `Select Participant 2 (Enter number, or 0 for empty):\n\n${pList}`, '', async (p2Index) => {
          ...
          try {
              ...
              const data = await response.json();
              if (data.ok) {
                  showModal('Success', 'Bracket updated successfully!', () => loadEventManagement());
              } else {
                  showModal('Error', data.message);
              }
          } catch (error) {
              console.error('Error updating bracket:', error);
              showModal('Error', 'Failed to update bracket');
          }
        });
      });
    });
...
async function updateMatchResults(eventId) {
  ...
  showInputModal('Update Match Result', 'Enter match ID to update (e.g., R0-M0):', '', async (matchId) => {
    if (!matchId) return;
    ...
    showInputModal('Select Winner', `Select winner:\n1. ${p1Name}\n2. ${p2Name}\n\nEnter 1 or 2:`, '', async (winner) => {
      if (!winner) return;

      const winnerId = winner === '1' ? match.participant1?.id : match.participant2?.id;

      if (!winnerId) {
        showModal('Error', 'Invalid selection');
        return;
      }
      ...
      try {
        ...
        if (data.ok) {
          showModal('Success', 'Match result updated!', () => loadEventManagement());
        }
      } catch (error) {
        console.error('Error updating match result:', error);
        showModal('Error', 'Failed to update match result');
